...
interface CardMediaProps {
  ...
  logoAlt?: string;
  icon?: IconComponent;
  /** Short text fallback for products that do not have an uploaded image. */
  label?: string;
  size?: number;
  className?: string;
}

function CardMedia({ logo, logoAlt, icon: Icon, label, size = 22, className }: CardMediaProps) {
  const { orientation } = useContext(CardContext);
  const shape = useShape();
  const sizeClasses = useSize();
  const compact = sizeClasses.variant === "compact";
  ...
  if (label) {
    return (
      <span
        ...
        aria-hidden="true"
        className={cn(
          "inline-flex size-10 shrink-0 items-center justify-center bg-surface-3 text-base font-medium tracking-[-0.04em] text-foreground",
          shape.bg,
          wrap
        )}
      >
